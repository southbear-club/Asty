...
public:
    ...
    void stat(slab_stat_t &st) {
        ...
        LOG(INFO) << "used_size : " << st.used_size << " bytes\n";
        LOG(INFO) << "used_pct : " << st.used_pct << " bytes\n";

        LOG(INFO) << "total page count : " << st.pages << "\n";
        LOG(INFO) << "free page count : " << st.free_page << "\n";

        LOG(INFO) << "small slab use page : " << st.p_small
                  << ", \tbytes : " << st.b_small << "\n";
        LOG(INFO) << "exact slab use page : " << st.p_exact
                  << ", \tbytes : " << st.b_exact << "\n";
        LOG(INFO) << "big slab use page : " << st.p_big
                  << ", \tbytes : " << st.b_big << "\n";
        LOG(INFO) << "page slab use page : " << st.p_page
                  << ", \tbytes : " << st.b_page << "\n";

        LOG(INFO) << "max free pages : " << st.max_free_pages << "\n";
    }
...
private:
        ...
        switch (type) {
            ...
            case NCX_SLAB_PAGE:
                ...
                if (slab == NCX_SLAB_PAGE_FREE) {
                    LOG(WARNING) << "page is already free\n";
                    goto fail;
                }

                if (slab == NCX_SLAB_PAGE_BUSY) {
                    LOG(WARNING) << "pointer to wrong page\n";
                    goto fail;
                }
    ...
    wrong_chunk:
        ...
        LOG(ERROR) << "pointer to wrong chunk\n";

        goto fail;

    chunk_already_free:

        LOG(ERROR) << "chunk is already free\n";
    ...
    void init(void) {
        ...
        pool_->start = (uint8_t *)asty_align_ptr(
